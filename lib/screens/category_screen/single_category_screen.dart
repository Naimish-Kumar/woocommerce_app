...
import 'package:woocommerce_app/screens/search_product_screen.dart';
import 'package:woocommerce_app/widgets/product_greed_view_widget.dart';
                          ...
                          if (snapshot.isNotEmpty) {
                            ...
                            if (filterDataModel.isFiltered) {
                              ...
                              for (var elementSnapShot in snapshot) {
                                final allVariationProducts = ref.watch(
                                    getSingleProductVariation(
                                        elementSnapShot.id!.toInt()));
                                allVariationProducts.when(data: (snapData) {
                                  for (var element in snapData) {
                                    for (var elementAtt
                                        in element.attributes!) {
                                      if (elementAtt.name?.toUpperCase() ==
                                          'COLOR') {
                                        if (!variationsColor
                                            .contains(elementAtt.option)) {
                                          variationsColor.add(
                                              elementAtt.option.toString());
                                        }
                                      }
                                      if (elementAtt.name?.toUpperCase() ==
                                          'SIZE') {
                                  ...
                                  if (elementSnapShot.type != 'simple') {
                                    ...
                                    for (var element in snapData) {
                                      ...
                                      for (var elementAtt
                                          in element.attributes!) {
                                        if (elementAtt.option ==
                                            filterDataModel.selectedColor)
                                          numberOfTrue++;
                                        if (elementAtt.option ==
                                            filterDataModel.selectedSize)
                                          numberOfTrue++;
                                      }
                                      if (filterDataModel.selectedColor !=
                                              'no data' &&
                                          filterDataModel.selectedSize !=
                                              'no data' &&
                                          numberOfTrue == 2) {
                                        showVariationProduct = true;
                                      } else if (filterDataModel
                                                  .selectedColor ==
                                              'no data' &&
                                          filterDataModel.selectedSize ==
                                              'no data') {
                                    ...
                                    if (snapData[0].salePrice.toInt().isBetween(
                                            filterDataModel.selectedPrice.start,
                                            filterDataModel
                                                .selectedPrice.end) &&
                                        (filterDataModel.isFiltered
                                            ? showVariationProduct
                                            : true)) {
                                      filteredProducts.add(elementSnapShot);
                                    }
                                  } else {
                                    if (elementSnapShot.salePrice
                                            .toInt()
                                            .isBetween(
                                                filterDataModel
                                                    .selectedPrice.start,
                              ...
                              return GridView.builder(
                                ...
                                itemBuilder: (BuildContext ctx, index) {
                                  ...
                                  return allVariationProducts.when(
                                      data: (snapData) {
                                    for (var element in snapData) {
                                      for (var elementAtt
                                          in element.attributes!) {
                                    ...
                                    if (snapshot[index].type != 'simple' &&
                                      ...
                                      int discount = discountGenerator(
                                          snapData[0].regularPrice.toString(),
                                          snapData[0].salePrice.toString());
                                      return ProductGreedShow(
                                      ...
                                      int discount = discountGenerator(
                                          snapshot[index]
                                              .regularPrice
                                              .toString(),
                                          snapshot[index].salePrice.toString());
                                      return ProductGreedShow(
                                        productModel: snapshot[index],
                                        discountPercentage: discount.toString(),
                                        isSingleView: isSingleView,
